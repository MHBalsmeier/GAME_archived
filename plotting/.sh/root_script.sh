...
do
python plotting/maps.py $run_span ${plot_intervals_list[$i]} ${disp_level_list[$i]} ${disp_shortname_list[$i]} $grid_props_file $fig_save_path $output_dir ${projections_list[$i]} $run_id ${uniform_colormap_list[$i]} ${coastlines_on_list[$i]} ${scope_list[$i]}
done
