...
int manage_pressure_gradient(State *state, Grid *grid, Dualgrid *dualgrid, Diagnostics *diagnostics, Forcings *forcings, Interpolate_info *interpolation, Diffusion_info *diffusion_info, Config_info *config_info, int no_step_rk)
{
    ...
    double rho_h, pressure_value_d, pressure_value_v, pressure_value, pot_temp_v;
    #pragma omp parallel for private(rho_h, pressure_value_d, pressure_value_v, pressure_value, pot_temp_v)
	...
	for (int i = 0; i < NO_OF_SCALARS; ++i)
	{
		if (config_info -> tracers_on == 1)
			diagnostics -> c_h_p_field[i] = spec_heat_cap_diagnostics_p(state -> density_dry[i], state -> tracer_densities[NO_OF_CONDENSATED_TRACERS*NO_OF_SCALARS + i]);
		else
