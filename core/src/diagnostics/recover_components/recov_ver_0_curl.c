...
int recov_ver_0_curl(Curl_field in_field, int layer_index, int h_index, double *component, Grid *grid)
{
    ...
    *component = 0;
    ...
    {
		for (int i = 0; i < 6; ++i)
		    *component += grid -> recov_ver_0_curl_weight[6*h_index + i]*in_field[layer_index*(NO_OF_DUAL_VECTORS_H + NO_OF_VECTORS_H) + grid -> recov_ver_index[6*h_index + i]];
	}
	else
	{
		for (int i = 0; i < 6; ++i)
		{
			...
			unmodified_value = grid -> recov_ver_0_curl_weight[6*h_index + i]*in_field[layer_index*(NO_OF_DUAL_VECTORS_H + NO_OF_VECTORS_H) + grid -> recov_ver_index[6*h_index + i]];
