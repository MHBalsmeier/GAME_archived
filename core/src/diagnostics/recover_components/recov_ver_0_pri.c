...
This is a recovery function at vertical vector points. The x component of a Vector_field is diagnozed here. If layer_index >= NUMBER_OF_LAYERS - NUMBER_OF_ORO_LAYERS, a vertical interpolation of the input field onto the desired z coordinate must be done.
*/
    ...
    else
    {
		...
		else
		{
		    for (int i = 0; i < 6; ++i)
		    {
		        ...
		        *component += z_upper_weight*grid -> recov_ver_0_pri_weight[6*h_index + i]*in_field[(layer_index - 1)*NUMBER_OF_VECTORS_PER_LAYER + NUMBER_OF_VECTORS_V + grid -> recov_ver_index[6*h_index + i]];
		        *component += (1 - z_upper_weight)*grid -> recov_ver_0_pri_weight[6*h_index + i]*in_field[layer_index*NUMBER_OF_VECTORS_PER_LAYER + NUMBER_OF_VECTORS_V + grid -> recov_ver_index[6*h_index + i]];
	        }
		}
    }
    return 0;
