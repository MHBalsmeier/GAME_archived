...
int recov_ver_1_pri(Vector_field in_field, int layer_index, int h_index, double *component, Grid *grid)
{
    *component = 0;
    ...
    if (layer_index < NO_OF_LAYERS - NO_OF_ORO_LAYERS)
    {
		...
		{
		    for (int i = 0; i < 6; ++i)
		        *component += grid -> recov_ver_1_pri_weight[6*h_index + i]*in_field[NO_OF_VECTORS_V + grid -> recov_ver_index[6*h_index + i]];
		}
		else
		{
		    for (int i = 0; i < 6; ++i)
		    {
		        *component += 0.5*grid -> recov_ver_1_pri_weight[6*h_index + i]*in_field[(layer_index - 1)*NO_OF_VECTORS_PER_LAYER + NO_OF_VECTORS_V + grid -> recov_ver_index[6*h_index + i]];
		        *component += 0.5*grid -> recov_ver_1_pri_weight[6*h_index + i]*in_field[layer_index*NO_OF_VECTORS_PER_LAYER + NO_OF_VECTORS_V + grid -> recov_ver_index[6*h_index + i]];
		    }
		}
    }
    else
    {
		...
		else
		{
		    for (int i = 0; i < 6; ++i)
		    {
		        ...
		        *component += z_upper_weight*grid -> recov_ver_1_pri_weight[6*h_index + i]*in_field[(layer_index - 1)*NO_OF_VECTORS_PER_LAYER + NO_OF_VECTORS_V + grid -> recov_ver_index[6*h_index + i]];
		        *component += (1 - z_upper_weight)*grid -> recov_ver_1_pri_weight[6*h_index + i]*in_field[layer_index*NO_OF_VECTORS_PER_LAYER + NO_OF_VECTORS_V + grid -> recov_ver_index[6*h_index + i]];
	        }
		}
    }
    return 0;
