...
int curl(Vector_field in_field, Dual_vector_field out_field, Grid *grid, Dualgrid *dualgrid)
{
    ...
    double distance_0, distance_1, distance_2, distance_3;
    for (int i = 0; i < NUMBER_OF_DUAL_VECTORS; ++i)
    {
        ...
        if (h_index >= NUMBER_OF_DUAL_VECTORS_H)
        {
            ...
            out_field[i] = 1/dualgrid -> area[i]*(grid -> normal_distance[index_0]*sign_0*in_field[index_0] + grid -> normal_distance[index_1]*sign_1*in_field[index_1] + grid -> normal_distance[index_2]*sign_2*in_field[index_2]);
        }
        else
        {
