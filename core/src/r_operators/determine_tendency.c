...
int tendency(State *current_state, State *state_tendency, Grid *grid, Dualgrid *dualgrid)
{
    ...
    {
        retval = calc_diffusion_coeff((*temperature)[i], mean_particle_mass, current_state -> density[i], eff_particle_radius, &mass_molecular_diffusion_coeff);
    ...
    for (int i = 0; i < NUMBER_OF_SCALARS; ++i)
    {
        retval = calc_diffusion_coeff((*temperature)[i], mean_particle_mass, current_state -> density[i], eff_particle_radius, &temperature_molecular_diffusion_coeff);
        ...
        state_tendency -> density_pot_temp[i] = -(*density_pot_temp_flux_divergence)[i] + current_state -> density_pot_temp[i]/current_state -> density[i]*mass_diffusion_coeff*(*laplace_density)[i] + 1/((*exner_pressure)[i]*C_P)*heat_power_density;
    }
    ...
    for (int i = 0; i < NUMBER_OF_VECTORS; ++i)
    {
        ...
        if (i < NUMBER_OF_VECTORS_V || i >= NUMBER_OF_VECTORS - NUMBER_OF_VECTORS_V)
            ...
            state_tendency -> wind[i] = -(*m_pressure_gradient_acc)[i] + (*abs_curl_tend)[i] - 0.5*(*m_e_kin_tend_2)[i] + grid -> gravity[i] + viscosity_coeff*(*laplace_wind_field)[i];
    }
