	...
	*/
	...
	for (int i = 0; i < 3; ++i)
		...
		*/
		...
		linear_combine_two_states(state_0, state_p1, state_tendency_precond, -(3 - i)/delta_t, (3 - i)/delta_t);
		step_counter = vertical_sound_wave_solver(state_0, state_p1, state_tendency_precond, state_tendency, delta_t/(3 - i), grid, temp_gradient_times_c_h_p, pressure_gradient_acc_1);
		//printf("%d\n", step_counter);
		three_band_solver_ver_entropy_gas(state_0, state_p1, state_tendency, delta_t/(3 - i), grid);
