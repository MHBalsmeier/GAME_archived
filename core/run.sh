...
then
mpirun -np $number_of_cpus $game_home_dir/core/game $run_span $write_out_interval $cfl_margin $momentum_diff $rad_on $operator $write_out_mass_dry_integral $write_out_entropy_gas_integral $write_out_energy_integral $temperature_diff_h $radiation_delta_t $start_year $start_month $start_day $start_hour $temperature_diff_v $run_id $write_out_linearized_entropy_gas_integral $toa $orography_id $ideal_input_id $mass_dry_diff_h $mass_dry_diff_v $grib_output_switch $netcdf_output_switch $pressure_level_output_switch $flight_level_output_switch $model_level_output_switch $surface_output_switch $orography_layers $type_of_vertical_grid $rk_order $assume_lte $adv_sound_ratio
fi
