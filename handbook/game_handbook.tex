...
Since the files \texttt{src/enum\_and\_typedefs.h} and \texttt{src/settings.c} are part of the model's source code, the model must be recompiled if something is changed in them. Alternatively, one can compile several eecutables and name them according to their configuration.

\lstinputlisting[language = Bash, caption = {Example input file.}, frame = on]{../run_scripts/ullrich_moist.sh}\label{lst:input_file_example}
