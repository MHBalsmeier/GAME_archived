...
int main(int argc, char *argv[])
{
    ...
    */
    ...
    ---------------------------
    */
    config -> total_run_span = strtod(argv[1], NULL);
    config_io -> write_out_interval = strtod(argv[2], NULL);
    config -> dt_parameter = strtod(argv[3], NULL);
    config -> momentum_diff_h = strtod(argv[4], NULL);
    config -> momentum_diff_v = strtod(argv[5], NULL);
    config -> rad_on = strtod(argv[6], NULL);
    config_io -> write_out_mass_integrals = strtod(argv[7], NULL);
    config_io -> write_out_rhotheta_integral = strtod(argv[8], NULL);
    config_io -> write_out_energy_integrals = strtod(argv[9], NULL);
    config -> temperature_diff_h = strtod(argv[10], NULL);
    config -> radiation_delta_t = strtod(argv[11], NULL);
    config_io -> year = strtod(argv[12], NULL);
    config_io -> month = strtod(argv[13], NULL);
    config_io -> day = strtod(argv[14], NULL);
    config_io -> hour = strtod(argv[15], NULL);
    strcpy(config_io -> month_string, argv[13]);
    strcpy(config_io -> day_string, argv[14]);
    strcpy(config_io -> hour_string, argv[15]);
    config -> temperature_diff_v = strtod(argv[16], NULL);
    strcpy(config_io -> run_id, argv[17]);
	config -> toa = strtod(argv[18], NULL);
	config -> oro_id = strtod(argv[19], NULL);
    config_io -> ideal_input_id = strtod(argv[20], NULL);
	...
	grid -> vert_grid_type = strtod(argv[27], NULL);
    ...
    */
    ...
    write_out(state_old, wind_h_10m_array, min_no_of_10m_wind_avg_steps, t_init, t_write, diagnostics, forcings, grid, dualgrid, config_io, config, soil);
    ...
    while (t_0 < t_init + config -> total_run_span + 300)
    {
    	...
    	*/
        ...
        if(t_0 + delta_t >= t_write + 300 && t_0 <= t_write + 300)
        {
            ...
            write_out(state_write, wind_h_10m_array, min_no_of_10m_wind_avg_steps, t_init, t_write, diagnostics, forcings, grid, dualgrid, config_io, config, soil);
