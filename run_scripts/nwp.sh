...
nwp_mode=1 # switches on or off the numerical weather prediction mode (calls data assimilation, special output, etc.)
...
momentum_diff=1 # turn on if you momentum diffusion
...
momentum_diff=1 # turn on if you momentum diffusion
tracers_on=1 # turn on if you want advect tracers and have phase transitions
...
write_out_interval=3600 # every how many seconds an output file will be created
write_out_mass_dry_integral=0 # If set to 1, the total dry mass of the atmosphere will be written out at every time step.
write_out_entropy_gas_integral=0 # If set to 1, the total entropy of the atmosphere will be written out at every time step.
write_out_linearized_entropy_gas_integral=0 # If set to 1, the total linearized entropy (proportional to density times potential temperature) of the atmosphere will be written out at every time step.
