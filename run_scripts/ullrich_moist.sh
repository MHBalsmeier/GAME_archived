...
test_id=5 # Must only be set if nwp_mode == 0. In this case, it specifies which test scenario to run.
overwrite_run_id=1 # this overwrites the otherwise automatically chosen run_id
run_id=ullrich_moist # run_id must only be set if overwrite_run_id == 0 (otherwise it is chosen automatically)
