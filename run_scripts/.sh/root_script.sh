...
then
./game $run_span $write_out_interva $momentum_diff_h $momentum_diff_v $rad_on $soil_on $no_rad_moisture_layers $write_out_integrals $temperature_diff_h $start_year $start_month $start_day $start_hour $temperature_diff_v $run_id $orography_id $ideal_input_id $grib_output_switch $netcdf_output_switch $pressure_level_output_switch $model_level_output_switch $surface_output_switch $assume_lte $delta_t_between_analyses $damping_start_height_over_toa $damping_coeff_max $explicit_boundary_layer $tracer_diff_h $tracer_diff_v
fi
